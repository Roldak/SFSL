...
#define RESTORE_TRAIT(trait, node) \
    ...
    T* update(T* old, Args... args) {
        ...
        SAVE_TRAIT(CacheableCreatedType, old)
        SAVE_TRAIT(CanHoldDefaultType, old)
        ...
        *old = T(std::forward<Args>(args)...);
        ...
        RESTORE_TRAIT(CacheableCreatedType, old)
        RESTORE_TRAIT(CanHoldDefaultType, old)
...
#define IMPL_TEMPLATED_TRAIT_SAVER_RESTORER(trait, type) \
    ...
    IMPL_TRAIT_SAVER_RESTORER(CacheableCreatedType, ast::HasCacheableCreatedType)
    IMPL_TRAIT_SAVER_RESTORER(CanHoldDefaultType, ast::CanHoldDefaultType)

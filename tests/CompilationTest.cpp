...
CompilationTest::CompilationTest(const std::string& name, const std::string& source,
                                 bool shouldCompile, const std::string& lastPhase)
    : AbstractTest(name), _source(source), _shouldCompile(shouldCompile),
      _lastPhase(lastPhase), _ppl(Pipeline::createDefault()) {
